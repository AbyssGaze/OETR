...
def main(opt):
    ...
    if opt.debug:
        ...
        opt.save_path = Path(f'./OUTPUT/SADNet/checkpoints/{cfg.OUTPUT}/' + timestamp)
    ...
    if cfg.SADNET.CHECKPOINT:
        model.load_state_dict(torch.load(cfg.SADNET.CHECKPOINT, map_location='cpu'))
    ...
    if opt.validation:
        ...
        if cfg.DATASET.VAL.DATA_SOURCE == 'megadepth_ms':
            validation_dataset.build_dataset(cfg.DATASET.VAL.SCALES[0], cfg.DATASET.VAL.SCALES[1], 
    ...
    for epoch in range(opt.epoch):
        model.float().train()#double and float
        
        if cfg.DATASET.TRAIN.DATA_SOURCE == 'megadepth_ms':
            scales = random.sample(cfg.DATASET.TRAIN.SCALES, 2)
            training_dataset.build_dataset(scales[0], scales[1], cfg.SADNET.BACKBONE.STRIDE)
            if opt.local_rank == 0:
                logger.info(scales)
        else:
        ...
        training_dataloader = torch.utils.data.DataLoader(training_dataset, batch_size=opt.batch_size, 
        ...
        for i, batch in enumerate(training_dataloader):
            ...
            if opt.local_rank == 0 and i % 50 ==0:
                ...
                logger.info('Epoch [{}][{}/{}], lr: {:E}, loss: {:.5f}, {}'.format(
                    epoch, i, len(training_dataloader), scheduler.get_last_lr()[0], loss, info))

                writer.add_scalar("Loss/train", loss.item(), i + epoch*len(training_dataloader))
                if cfg.DATASET.TRAIN.VIZ:
                    bbox1 = data['pred_bbox1'][0].detach().cpu().numpy().astype(int)
                    bbox2 = data['pred_bbox2'][0].detach().cpu().numpy().astype(int)
                    gt_bbox1 = batch['overlap_box1'][0].cpu().numpy().astype(int)
                    gt_bbox2 = batch['overlap_box2'][0].cpu().numpy().astype(int)
                    viz_name = os.path.join(str(opt.save_path), 'train_{}_{}_'.format(epoch, i)+batch['file_name'][0])
                    if 'pred_center1' in data.keys():
                        visualize_centerness_overlap_gt(batch['image1'][0].cpu().numpy() * 255, bbox1, gt_bbox1, data['pred_center1'][0].detach().cpu().numpy(),
                                    batch['image2'][0].cpu().numpy() * 255, bbox2, gt_bbox2, data['pred_center2'][0].detach().cpu().numpy(),viz_name)
                    else:
                        visualize_overlap_gt(batch['image1'][0].cpu().numpy() * 255, bbox1, gt_bbox1, 
                                    batch['image2'][0].cpu().numpy() * 255, bbox2, gt_bbox2, viz_name)
        if opt.local_rank == 0 and opt.validation:
            model.eval()
            evaluate(model, validation_dataloader, logger, opt.save_path, 
                    iou_thrs=np.arange(0.5, 0.96, 0.05), epoch=epoch, oiou=cfg.DATASET.VAL.OIOU, viz=cfg.DATASET.VAL.VIZ)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--config_path', type=str, default='assets/megadepth/config.py',
