...
@torch.no_grad()
def evaluate(model, dataloader, logger, save_path,
             iou_thrs=np.arange(0.5, 0.96, 0.05),
             epoch=0, oiou=False, viz=False):
    ious = []
    oious = []
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        if not oiou:
            ious1 = bbox_overlaps(batch['overlap_box1'],
                                  data['pred_bbox1'].cpu(), is_aligned=True)
            ious2 = bbox_overlaps(batch['overlap_box2'],
                                  data['pred_bbox2'].cpu(), is_aligned=True)
        else:
            ious1 = bbox_overlaps(batch['overlap_box1'],
                                  data['pred_bbox1'].cpu(), is_aligned=True)
            ious2 = bbox_overlaps(batch['overlap_box2'],
        ...
        if i % 10 == 0 and viz:
            ...
            viz_name = os.path.join(
                str(save_path), 'epoch'+str(epoch)+'_'+batch['file_name'][0])
            if 'pred_center1' in data.keys():
                visualize_centerness_overlap_gt(
                    batch['image1'][0].cpu().numpy() * 255, bbox1,
                    gt_bbox1, data['pred_center1'][0].cpu().numpy(),
                    batch['image2'][0].cpu().numpy() * 255, bbox2,
                    gt_bbox2, data['pred_center2'][0].cpu().numpy(), viz_name)
            else:
...
@torch.no_grad()
def evaluate_dummy(model, dataloader, logger, save_path,
                   iou_thrs=np.arange(0.5, 0.96, 0.05),
                   epoch=0, oiou=False, viz=False):
    ious = []
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        if not oiou:
            ious1 = bbox_overlaps(batch['overlap_box1'],
                                  pred_bbox1.cpu(), is_aligned=True)
            ious2 = bbox_overlaps(batch['overlap_box2'],
                                  pred_bbox2.cpu(), is_aligned=True)
        else:
        ...
        if True:
            ...
            viz_name = os.path.join(
                str(save_path), 'epoch'+str(epoch)+'_'+batch['file_name'][0])
            visualize_overlap_gt(
                batch['image1'][0].numpy() * 255, bbox1, gt_bbox1,
                batch['image2'][0].numpy() * 255, bbox2, gt_bbox2,
                viz_name)
    eval_recalls(ious, iou_thrs, logger)
