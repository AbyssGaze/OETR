...
def visualize_box(image1, bbox1, points1, depth1, image2, bbox2, points2, depth2, output):
    ...
    # depth_viz = cv2.hconcat([np.stack([depth1]*3, -1)*10, np.stack([depth2]*3, -1)*10])
...
def overlap_box(K1, depth1, pose1, bbox1, ratio1,
    ...
    valid_uv1 = torch.stack((u1[valid_corners], v1[valid_corners])).numpy().astype(int)
    ...
    box1 = [(valid_uv1[0].min(), valid_uv1[1].min()), (valid_uv1[0].max(), valid_uv1[1].max())]
    box2 = [(valid_uv2[0].min(), valid_uv2[1].min()), (valid_uv2[0].max(), valid_uv2[1].max())]
    return box1, valid_uv1, box2, valid_uv2
    

def numpy_overlap_box(K1, depth1, pose1, bbox1, ratio1,
                      K2, depth2, pose2, bbox2, ratio2):
...
class MegaDepthDataset(Dataset):
    ...
    def caculate_depth(pos, depth):
        ...
        pos = torch.cat([pos[0, :][ids].view(1, -1), 
                        pos[1, :][ids].view(1, -1)], dim=0)
    ...
    def recover_pair(self, pair_metadata):
        ...
        assert gray1.shape[0] == depth1.shape[0] and gray1.shape[1] == depth1.shape[1]
        ...
        assert gray2.shape[0] == depth2.shape[0] and gray2.shape[1] == depth2.shape[1]
        ...
        if self.with_mask:
            ...
            mask1 = np.zeros(1)
            mask2 = np.zeros(1)
        match_file_name = pair_metadata["image_path1"].split('/')[-1] + '_' + pair_metadata["image_path2"].split('/')[-1]
...
def main(scene_list_path, scene_info_path, dataset_path, batch_size, num_workers, local_rank=0):
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        if i % 10 != 0:
            continue
        # box1, valid_uv1, box2, valid_uv2 = overlap_box(batch['intrinsics1'][0], batch['depth1'][0], batch['pose1'][0],
        #                         batch['bbox1'][0], batch['ratio1'][0], 
        #                         batch['intrinsics2'][0], batch['depth2'][0], batch['pose2'][0],
        #                         batch['bbox2'][0], batch['ratio2'][0])

        box1, valid_uv1, box2, valid_uv2 = numpy_overlap_box(batch['intrinsics1'][0].numpy(), 
                                batch['depth1'][0].numpy(), batch['pose1'][0].numpy(),
                                batch['bbox1'][0].numpy(), batch['ratio1'][0].numpy(), 
                                batch['intrinsics2'][0].numpy(), batch['depth2'][0].numpy(), batch['pose2'][0].numpy(),
                                batch['bbox2'][0].numpy(), batch['ratio2'][0].numpy())
        visualize_box(batch['image0'][0]*255, box1, valid_uv1, batch['depth1'][0], 
                    batch['image1'][0]*255, box2, valid_uv2, batch['depth2'][0], batch['file_name'][0])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--scene_info_path', type=str, default='assets/megadepth/',
