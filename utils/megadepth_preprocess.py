...
def scale_diff(bbox0, bbox1, depth0, depth1):
    ...
    # image_h_scale = max(depth0.shape[0]/(bbox0[3] - bbox0[1]), depth1.shape[0]/(bbox1[3] - bbox1[1]))
    # image_w_scale = max(depth0.shape[1]/(bbox0[2] - bbox0[0]), depth1.shape[1]/(bbox1[2] - bbox1[0]))
...
def process_scene(scene, datasets, pairs_per_scene=3000, existed_pairs=[],
                min_overlap_ratio=0.1, max_overlap_ratio = 0.7, 
                max_scale_ratio=100):
    data = ''
    ...
    for pair_idx in selected_ids:
        ...
        if bbox0.max() > 0 and bbox1.max() > 0 and scale_diff(bbox0, bbox1, depth0, depth1) > 2:
            ...
            info = '{} {} {} {} {} {} {} {} {} {}\n'.format(image_paths[idx0], depth_paths[idx0], 
                                                    K0, pose0, bbox0, image_paths[idx1], depth_paths[idx1], 
                                                    K1, pose1, bbox1)
            data += info
            valid += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--scenes', type=str, default='assets/megadepth/validation.txt',
        help='Path to the list of scenes')
    parser.add_argument(
        '--datasets', type=str, default='assets/megadepth/',
        help='Path to the list of image pairs')
    parser.add_argument(
        '--input_pairs', type=str, default='assets/megadepth/megadepth_0.4.txt',
        help='Path to the list of image pairs')
    parser.add_argument(
        '--max_overlap_ratio', type=float, default=0.7,
    ...
    for scene in scenes:
        pool.apply_async(process_scene, (scene, opt.datasets, opt.pairs_per_scene, existed_pairs), callback=megadepth_callback)
