...
def visualize_mask(image1, bbox1, mask1, depth1, image2, bbox2, mask2, depth2,
                   output):
    bbox1 = bbox1.numpy()
    bbox2 = bbox2.numpy()
...
class MegaDepthPairsResizeDataset(Dataset):
    ...
    def overlap_box_simple(self,
        ...
        box1 = [
            overlap_bbox1[0] - bbox1[1], overlap_bbox1[1] - bbox1[0],
            overlap_bbox1[2] - bbox1[1], overlap_bbox1[3] - bbox1[0]
        ]
        box2 = [
            overlap_bbox2[0] - bbox2[1], overlap_bbox2[1] - bbox2[0],
            overlap_bbox2[2] - bbox2[1], overlap_bbox2[3] - bbox2[0]
        ]
        # overlap with outsize images
        resize_crop_bbox1 = np.array([
            box1[0].clip(0, w), box1[1].clip(0, h), box1[2].clip(0, w),
            box1[3].clip(0, h)
        ])
        resize_crop_bbox2 = np.array([
            box2[0].clip(0, w), box2[1].clip(0, h), box2[2].clip(0, w),
            box2[3].clip(0, h)
        ])
    ...
    def recover_pair(self, pair_metadata):
        ...
        return (image1, depth1, intrinsics1, pose1, bbox1, resize_ratio1,
                overlap_box1, overlap_mask1, mask1, image2, depth2,
                intrinsics2, pose2, bbox2, resize_ratio2, overlap_box2,
                overlap_mask2, mask2, match_file_name, central_match,
                overlap_valid)
...
def main_overlap(pairs_list_path,
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        if i % 10:
            continue
        visualize_mask(batch['image1'][0] * 255, batch['overlap_box1'][0],
                       batch['overlap_mask1'][0], batch['depth1'][0],
                       batch['image2'][0] * 255, batch['overlap_box2'][0],
                       batch['overlap_mask2'][0], batch['depth2'][0],
                       batch['file_name'][0])
