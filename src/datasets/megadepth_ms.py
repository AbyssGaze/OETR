...
from .utils import visualize_mask, visualize_box
...
class MegaDepthMultiScaleDataset(Dataset):
    ...
    def resize_pad_dataset(self, img, scale=(640.0, 640.0), pad_scale=(640, 640), depth=False):
        ...
        # resize w*h
        if depth:
            img_resize = cv2.resize(img, new_size, interpolation = cv2.INTER_NEAREST)
            ...
            #pad image
            padded = np.zeros((pad_scale[1], pad_scale[0], img.shape[2]), dtype=img.dtype)
            padded[:img_resize.shape[0], :img_resize.shape[1], :] = img_resize
            mask = np.zeros((int(pad_scale[1]/stride), int(pad_scale[0]/stride)), dtype=bool)
            mask[:int(img_resize.shape[0]/stride), :int(img_resize.shape[1]/stride)] = True
    ...
    def overlap_box_simple(self, overlap1, ratio1,
                        overlap2, ratio2):

        overlap_bbox1 = overlap1 * np.tile(ratio1, 2)
        overlap_bbox2 = overlap2 * np.tile(ratio2, 2)
        return overlap_bbox1, overlap_bbox2, np.zeros((1, 1)), np.zeros((1, 1)), True
    ...
    def overlap_box(self, K1, depth1, pose1, bbox1, ratio1,
        ...
        if valid_uv1.shape[1] == 0 or valid_uv2.shape[1] == 0:
            return np.array([0]*4), np.array([0]*4), np.zeros((h1, w1)), np.zeros((h2, w2)), False
    ...
    def build_dataset(self, img_scale1=[640, 640], img_scale2=[640, 640], size_divisor=32):
        if not self.train:
        ...
        self.img_scale1_divisor = [math.ceil(img_scale1[0]/size_divisor)*size_divisor, 
                                    math.ceil(img_scale1[1]/size_divisor)*size_divisor]
        self.img_scale2_divisor = [math.ceil(img_scale2[0]/size_divisor)*size_divisor, 
    ...
    def recover_pair(self, pair_metadata):
        ...
        image1, resize_mask1, resize_ratio1 = self.resize_pad_dataset(image1, self.img_scale1, self.img_scale1_divisor)
        image2, resize_mask2, resize_ratio2 = self.resize_pad_dataset(image2, self.img_scale2, self.img_scale2_divisor)

        depth1, _, _ = self.resize_pad_dataset(depth1, self.img_scale1, self.img_scale1_divisor, True)
        depth2, _, _ = self.resize_pad_dataset(depth2, self.img_scale2, self.img_scale2_divisor, True)
        ...
        # overlap_box1, overlap_box2, overlap_mask1, overlap_mask2, overlap_valid = self.overlap_box_simple(
        #                 pair_metadata['overlap1'], resize_ratio1,
        #                 pair_metadata['overlap2'], resize_ratio2,
        #                 )
...
def main_overlap(pairs_list_path, dataset_path, batch_size, num_workers, local_rank=0):
    ...
    for epoch in range(1):
        ...
        dataset.build_dataset(img_scale1=random_scales[0], img_scale2=random_scales[1])
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, 
                                                num_workers=num_workers, shuffle=False)
        for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            visualize_mask(batch['image1'][0]*255, batch['overlap_box1'][0], batch['overlap_mask1'][0], batch['depth1'][0], 
                        batch['image2'][0]*255, batch['overlap_box2'][0], batch['overlap_mask2'][0], batch['depth2'][0], 
                        str(epoch)+'_' + batch['file_name'][0], fig=True)
